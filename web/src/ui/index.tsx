...
export function Card({
  ...
  title?: ReactNode
  /** One line under the title, for the thing the title cannot say on its own. */
  subtitle?: ReactNode
  action?: ReactNode
  children: ReactNode
  style?: CSSProperties
      ...
      {(title || subtitle || action) && (
        ...
        >
          <div style={{ minWidth: 0 }}>
            ...
            {subtitle && (
              <p style={{ margin: '0.15rem 0 0', fontSize: '0.8rem', color: 'var(--muted)' }}>{subtitle}</p>
            )}
          </div>
          {action}
        </header>
...
export function Notice({
  ...
  children?: ReactNode
  action?: ReactNode
  /** When given, the notice can be closed. Errors on a till pile up otherwise. */
  onDismiss?: () => void
}) {
  const palette = {
