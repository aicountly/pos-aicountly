# AICOUNTLY auth workflow (POS)

How POS signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `pos.aicountly.com` (or `pos.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/pos?returnUrl={origin}/auth/callback`.
...
| POS host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `pos.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `pos.aicountly.com/api` |
| `pos.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `pos.gh.aicountly.com/api` |
...
`pos` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://pos.gh.aicountly.com/api/health
...
curl -i -X POST https://pos.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://pos.gh.aicountly.com/api/global/login   # expect 404
```
