...
POS for Aicountly — a React single-page app built with Vite and TypeScript,
with a small PHP API alongside it. Both halves deploy to cPanel.

| Environment | App | API |
| --- | --- | --- |
| Production | https://pos.aicountly.com | https://pos.aicountly.com/api |
| Sandbox | https://pos.gh.aicountly.com | https://pos.gh.aicountly.com/api |
...
(`https://pos.gh.aicountly.com/api`) so the token exchange has somewhere to
...
| `VITE_CONSOLE_API_BASE_URL` | Where the app launcher asks for product icons. Defaults to Console. `off` uses only the bundled tiles |
